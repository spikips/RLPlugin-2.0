# ghoul.py
# A script for automating a Ghoul slayer task using a cannon in Old School RuneScape via RuneLite plugin.
...
# No specific stand tile - stays wherever positioned.
# No looting.
# On task completion: Navigates to Slayer Master Vannaka in Edgeville Dungeon.
...
# Constants
CANNON_TILE = (3427, 3461, 0)  # Cannon position (x, y, plane)
...
def is_cannon_placed():
    ...
    fire_obj = get_closest_object(CANNON_NAME, "Fire", tile=CANNON_TILE, radius=10, disregard_canvas=True)
    pickup_obj = get_closest_object(CANNON_NAME, "Pick-up", tile=CANNON_TILE, radius=10, disregard_canvas=True)
...
def main():
    ...
    print("Starting Ghoul task with cannon (no fixed stand tile).")
