# hobgoblins.py
# A script for automating a hobgoblin slayer task using a cannon in Old School RuneScape via RuneLite plugin.
# Picks up rare drops: Snape grass seed, Loop half of a key, Tooth half of a key, Rune spear, Shield left half, Dragon spear.
...
from modules.core.plugin_client import cannon_data, minimap_tile_point, player, inventory, slayer_task_remaining, pick
...
# Constants
CANNON_TILE = (2447, 3363, 0)  # Cannon position (x, y, plane)
STAND_TILE = (2445, 3364, 0)   # Standing position (x, y, plane)
...
def place_cannon():
    ...
    if not on_tile:
        ...
        if (player_pos.get('x'), player_pos.get('y'), player_pos.get('plane')) != CANNON_TILE:
            ...
            mm_pos = minimap_tile_point(CANNON_TILE[0], CANNON_TILE[1])
            if mm_pos and isinstance(mm_pos, tuple) and len(mm_pos) == 2:
                mm_x, mm_y = mm_pos
                move(mm_x, mm_y, fast=True, sleep=True)
                select_menu_option(mm_x, mm_y, "Walk here")
                wait_till_character_stopped_moving()
            else:
                print("Failed to get minimap point. Skipping placement this attempt.")
                return False
...
from modules.utils.loot import pickup_closest_ground_item

def pick_loot():
    """Scan tiles around the cannon for rare loot and pick the first matching item found."""
    cx, cy, _ = CANNON_TILE
    for dx in range(-6, 7):      # Reasonable radius around cannon
        for dy in range(-6, 7):
            tx = cx + dx
            ty = cy + dy
            response = pick(tx, ty, size=5)
            if response and 'data' in response:
                data = response['data']
                items = data.get('items', [])
                if not items and data.get('item'):  # Handle possible single item case
                    items = [data]

                for item in items:
                    name = item.get('name', '')
                    if name in LOOT_ITEMS:
                        mp = item.get('middle_point')
                        if mp:
                            sx, sy = runelite_window(mp['x'], mp['y'])
                            move(sx, sy, fast=True, button='left', sleep=True)
                            print(f"Picked up {name} at tile ({tx}, {ty})")
                            wait_for_tick(3)  # Wait for pickup animation/network delay
                            return True
    return False
...
def main():
    ...
    print("Starting hobgoblin slayer task with cannon.")
