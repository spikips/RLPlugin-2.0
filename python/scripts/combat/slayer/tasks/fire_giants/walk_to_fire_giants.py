...
def walk_to_fire_giants():
    """Walk to the fire_giants location."""
    for i in range(5):
    ...
    # 3
    if not click_widget('35913797', sprite_id=1030, hidden=False, right_click=False, action=None, rand_x=5, rand_y=5, clicks=1, sleep_interval=(0, 0)):
    ...
    for i in range(10):
        if click_minimap_tile(1459, 9898, rand_x=2, rand_y=2, target_zoom=2.0):
            print("clicked minimap tile (1459, 9898)")
            for i in range(3):
                if camera(pitch=485, yaw=2044, zoom=477, speed=10):
                    break
                if i == 2:
                    exit("Failed to set camera")
        ...
        wait_for_next_tick()
        if i == 9:
            exit("Failed to click minimap tile (1459, 9898)")


    for i in range(3):
        if click_tile(1460, 9904, action="Walk here", tile_radius=20, right_click=False):
            if wait_till_character_stopped_moving():
                    break
            if i == 2:
                exit("Failed to walk to (1460, 9904)")
