import time
from modules.banking.bank_castlewars import bank_castlewars
from modules.player_data.ensure_correct_attack_style import ensure_correct_combat_style
from modules.player_data.prayer.toggle_prayer import toggle_prayer
from modules.utils.check_players import check_for_players
from modules.utils.click_minimap_tile import click_minimap_tile
from modules.player_data.tile_change import wait_for_tile_change
from modules.utils.hop import hop_to_random_world
from modules.utils.wait_for_tick import wait_for_next_tick, wait_for_tick
from modules.utils.camera import camera
from modules.widgets.widget import click_widget, click_widget_child
from modules.object_data.game_object import click_gameobject
from modules.player_data.wait_till_character_stops_moving import wait_till_character_stopped_moving
from modules.utils.automatic_scripting.small_functions import click_equipped_glory
from modules.player_data.check_run import click_run
from modules.utils.click_tile import click_tile
from scripts.combat.slayer.slayer_gear import get_target_gear


def go_to_bank():
    """Go to bank and get target gear and inventory."""
    target_gear = get_target_gear()

    target_inventory = {
    "Prayer potion": 5,
    "Amulet of glory": 1,
    "Ring of dueling": 2,
    "Cannon barrels": 1,
    "Cannon furnace": 1,
    "Cannon base": 1,
    "Cannon stand": 1,
    "Steel cannonball": "all",
    }

    bank_castlewars(target_gear=target_gear, target_inventory=target_inventory)
    ensure_correct_combat_style()

def walk_to_ice_giants():
    """Walk to the ice_giants location."""
    # Clicked jewelry: Amulet of glory(3) (action: Draynor Village) - Equipment tab open -> using equipped
    for i in range(5):
        if click_equipped_glory(action='Draynor Village'):
            wait_for_tile_change()
            wait_for_next_tick()
            break
        wait_for_next_tick()
        if i == 4:
            exit("Failed to click jewelry: Amulet of glory(3) (Draynor Village)")


    # 4
    for i in range(10):
        if click_minimap_tile(3077, 3251, rand_x=2, rand_y=2, target_zoom=2.0):
            print("clicked minimap tile (3077, 3251)")
            for i in range(3):
                if camera(pitch=450, yaw=0, zoom=400, speed=10):
                    break
                if i == 2:
                    exit("Failed to set camera")
            if wait_till_character_stopped_moving(required_idle_ticks=2):
                break
        wait_for_next_tick()
        if i == 9:
            exit("Failed to click minimap tile (3077, 3251)")


    # 6
    for i in range(10):
        if click_minimap_tile(3062, 3256, rand_x=2, rand_y=2, target_zoom=2.0):
            print("clicked minimap tile (3062, 3256)")
            # 5
            if not click_widget('35913795', sprite_id=1030, hidden=False, right_click=False, action=None, rand_x=0, rand_y=0, clicks=1, sleep_interval=(0, 0)):
                exit(f'click widget 35913795 failed, exiting... time: {time.strftime("%H:%M:%S")}')
            # 7
            for i in range(3):
                if camera(pitch=211, yaw=783, zoom=243, speed=10):
                    break
                if i == 2:
                    exit("Failed to set camera")
            if wait_till_character_stopped_moving(required_idle_ticks=2):
                break
        wait_for_next_tick()
        if i == 9:
            exit("Failed to click minimap tile (3062, 3256)")


    # 8
    for i in range(10):
        if click_minimap_tile(3035, 3236, rand_x=2, rand_y=2, target_zoom=2.0):
            print("clicked minimap tile (3035, 3236)")
            if wait_till_character_stopped_moving():
                break
        wait_for_next_tick()
        if i == 9:
            exit("Failed to click minimap tile (3035, 3236)")

    # 9
    for i in range(10):
        if click_minimap_tile(3008, 3214, rand_x=2, rand_y=2, target_zoom=2.0):
            print("clicked minimap tile (3008, 3214)")
            if wait_till_character_stopped_moving():
                break
        wait_for_next_tick()
        if i == 9:
            exit("Failed to click minimap tile (3008, 3214)")

    # 10
    for i in range(10):
        if click_minimap_tile(2999, 3192, rand_x=2, rand_y=2, target_zoom=2.0):
            print("clicked minimap tile (2999, 3192)")
            if wait_till_character_stopped_moving():
                break
        wait_for_next_tick()
        if i == 9:
            exit("Failed to click minimap tile (2999, 3192)")

    # 11
    for i in range(10):
        if click_minimap_tile(3001, 3168, rand_x=2, rand_y=2, target_zoom=2.0):
            print("clicked minimap tile (3001, 3168)")
            if wait_till_character_stopped_moving():
                break
        wait_for_next_tick()
        if i == 9:
            exit("Failed to click minimap tile (3001, 3168)")


    # 13
    for i in range(10):
        if click_minimap_tile(3008, 3153, rand_x=2, rand_y=2, target_zoom=2.0):
            print("clicked minimap tile (3008, 3153)")
            # 12
            for i in range(3):
                if camera(pitch=404, yaw=1058, zoom=243, speed=10):
                    break
                if i == 2:
                    exit("Failed to set camera")
            if wait_till_character_stopped_moving():
                wait_for_next_tick(3)
                break
        wait_for_next_tick()
        if i == 9:
            exit("Failed to click minimap tile (3008, 3153)")

    # 14
    # Object click: trapdoor -> Climb-down
    for i in range(5):
        if click_gameobject("1738", 'Climb-down', tile=(3007, 3149), radius=20):
            wait_till_character_stopped_moving(required_idle_ticks=5)
            break
        wait_for_next_tick()
        if i == 4:
            exit("Failed to click object (trapdoor, Climb-down)")


    for i in range(5):
        if click_widget_child('35913752', sprite_id=None, hidden=None, child_index=1, right_click=False, action=None):
            break
        wait_for_next_tick()
        if i == 4:
            exit("Failed to click dialogue option (look north) via child")


    # 17
    for i in range(10):
        if click_minimap_tile(2993, 9566, rand_x=2, rand_y=2, target_zoom=2.0):
            print("clicked minimap tile (2993, 9566)")
            if wait_till_character_stopped_moving():
                break
        wait_for_next_tick()
        if i == 9:
            exit("Failed to click minimap tile (2993, 9566)")

    # 18
    for i in range(10):
        if click_minimap_tile(2966, 9569, rand_x=2, rand_y=2, target_zoom=2.0):
            print("clicked minimap tile (2966, 9569)")
            # 24
            for i in range(3):
                if camera(pitch=452, yaw=957, zoom=496, speed=10):
                    break
                if i == 2:
                    exit("Failed to set camera")
            # 20     
            if not click_widget('35913797', sprite_id=1030, hidden=False, right_click=False, action=None, rand_x=0, rand_y=0, clicks=1, sleep_interval=(0, 0)):
                exit(f'click widget 35913797 failed, exiting... time: {time.strftime("%H:%M:%S")}')

            # 21
            for i in range(5):
                if click_widget('35454999', action='activate', hidden=False, right_click=False, rand_x=0, rand_y=0, clicks=1, sleep_interval=(0, 0)):
                    break
                wait_for_next_tick()
                if i == 4:
                    exit("Failed to activate prayer (protect from melee)")

            # 22
            if not click_widget('35913795', sprite_id=1030, hidden=False, right_click=False, action=None, rand_x=0, rand_y=0, clicks=1, sleep_interval=(0, 0)):
                exit(f'click widget 35913795 failed, exiting... time: {time.strftime("%H:%M:%S")}')

            if wait_till_character_stopped_moving():
                break
        wait_for_next_tick()
        if i == 9:
            exit("Failed to click minimap tile (2966, 9569)")

    # 25
    for i in range(3):
        if click_tile(2966, 9565, action="Walk here", tile_radius=20, right_click=False):
            click_run(enable=True)  # Enable run
            if wait_till_character_stopped_moving():
                    break
        if i == 2:
            exit("Failed to walk to (2966, 9565)")


def hop_if_players_nearby():
    """Hop worlds if players are detected nearby."""
    hop_tile = (2959, 9561, 0)
    stand_tile = (2967, 9566, 0)

    # hop until empty world
    max_hop_attempts = 15  # Safety limit to avoid infinite hopping
    for hop_attempt in range(max_hop_attempts):
        print(f"Checking for nearby players (attempt {hop_attempt + 1}/{max_hop_attempts})...")
        
        players_detected = check_for_players(radius=10, max_wait_ticks=2)
        
        if not players_detected:
            print("No players detected in 10 tile radius -> safe spot")
            break

        click_minimap_tile(hop_tile[0], hop_tile[1], target_zoom=2.0)
        wait_till_character_stopped_moving(required_idle_ticks=2)
        wait_for_next_tick(8)


        print("Players detected -> hopping to new P2P world")
        if hop_to_random_world('p2p'):
            print("hopped, rechecking")
            wait_for_tick(3)
            continue

        players_detected = check_for_players(radius=10, max_wait_ticks=2)

        if not players_detected:
            toggle_prayer('PROTECT_FROM_MELEE', activate=True)
            click_minimap_tile(stand_tile[0], stand_tile[1], target_zoom=2.0)
            wait_till_character_stopped_moving(required_idle_ticks=2)
            break


def run():
    """Execute the main sequence: go to bank, walk to location, and check for players."""
    go_to_bank()
    walk_to_ice_giants()
    hop_if_players_nearby()


if __name__ == "__main__":
    run()
