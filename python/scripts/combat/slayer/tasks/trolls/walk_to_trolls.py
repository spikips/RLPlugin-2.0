...
def walk_to_trolls():
    ...
    #6
    for i in range(5):
        if click_widget_child('62062601', sprite_id=None, hidden=False, child_index=7, right_click=False, action=None):
            wait_for_tile_change()
    ...
    # 1
    for i in range(10):
        if click_minimap_tile(1246, 3515, rand_x=1, rand_y=1, target_zoom=2.0):
            print("clicked minimap tile (1246, 3515)")
            for i in range(3):
                if camera(pitch=491, yaw=1779, zoom=347, speed=10):
                    break
                if i == 2:
                    exit("Failed to set camera")
        ...
        wait_for_next_tick()
        if i == 9:
            exit("Failed to click minimap tile (1246, 3515)")

    for i in range(3):
        if click_tile(1244, 3513, action="Walk here", tile_radius=20, right_click=False):
            if wait_till_character_stopped_moving():
                    break
            if i == 2:
                exit("Failed to walk to (1244, 3513)")


def hop_if_players_nearby():
    """Hop worlds if players are detected nearby."""
    hop_tile =  (1243, 3523, 0)
