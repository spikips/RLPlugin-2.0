...
def walk_to_hill_giants():
    """Walk to the hill_giants location."""
    for i in range(5):
    ...
    # 6
    for i in range(10):
        if click_minimap_tile(1435, 9884, rand_x=2, rand_y=2, target_zoom=2.0):
            print("clicked minimap tile (1435, 9884)")
            if wait_till_character_stopped_moving():
                break
        wait_for_next_tick()
        if i == 9:
            exit("Failed to click minimap tile (1435, 9884)")
