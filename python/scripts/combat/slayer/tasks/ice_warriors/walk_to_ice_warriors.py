...
def walk_to_ice_warriors():
    """Walk to the ice_warriors location."""
    ...
    # 2
    for i in range(3):
        if camera(pitch=415, yaw=0, zoom=284, speed=10):
            break
        if i == 2:
            exit("Failed to set camera")
    ...
    # 3
    for i in range(10):
        if click_minimap_tile(3023, 9580, rand_x=2, rand_y=2, target_zoom=2.0):
            print("clicked minimap tile (3023, 9580)")
            if wait_till_character_stopped_moving(required_idle_ticks=2):
                break
        wait_for_next_tick()
        if i == 9:
            exit("Failed to click minimap tile (3023, 9580)")


    for i in range(10):
        if click_minimap_tile(3035, 9582, rand_x=1, rand_y=1, target_zoom=2.0):
            print("clicked minimap tile (3035, 9582)")
            if wait_till_character_stopped_moving(required_idle_ticks=2):
                break
        wait_for_next_tick()
        if i == 9:
            exit("Failed to click minimap tile (3035, 9582)")
...
def hop_if_players_nearby():
    ...
    hop_tile = (3035, 9582, 0)
    stand_tile = (3052, 9585, 0)
