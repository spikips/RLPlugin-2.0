# new_cannon_spot.py
# A script similar to moss_giants.py but adapted for a new location with fixed camera settings
# and added poison handling via Antidote++ (prefers drinking from lowest dose first).
# Cannon tile: (3319, 9481, 0)
# Stand tile: (3321, 9483, 0)
# Fixed camera: pitch=306, yaw=1369, zoom=356, speed=10
...
from modules.player_data.is_poisoned import is_poisoned  # Added for poison check
...
# Constants
CANNON_TILE = (3319, 9481, 0)  # Cannon position (x, y, plane)
STAND_TILE = (3320, 9483, 0)   # Standing position (x, y, plane)
...
def drink_antidote():
    """Drink one dose of Antidote++ - prefers lowest dose first (1 → 4)."""
    inv_data = inventory()
    if not inv_data or 'data' not in inv_data or not inv_data['data']:
        return False

    for dose in range(1, 5):  # Check (1) first, then (2), (3), (4)
        target_name = f"Antidote++({dose})"
        for inv_item in inv_data['data']:
            name = inv_item.get('name', '').strip()
            if name == target_name:
                mp = inv_item['middle_point']
                sx, sy = runelite_window(mp['x'], mp['y'])
                move(sx, sy, fast=True, sleep=True, button='left')
                print(f"Drank {name} to cure poison")
                wait_for_tick(1)
                return True
    return False
...
def main():
    ...
    print("Starting cannon task at new location.")
    ...
    while True:
        ...
        if task_remaining == 0:
            ...
            print("Navigation complete - script ending.")
            break
        ...
        # Poison management - cure if poisoned
        if is_poisoned():
            if drink_antidote():
                print("Cured poison with Antidote++")
            else:
                print("Poisoned but no Antidote++ available")
