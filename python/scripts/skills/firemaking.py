...
def open_spellbook():
    ...
    for _ in range(open_timeout):
        ...
        spellbook_open = any(widget.get("id") == 35913798 and widget.get("spriteId", -1) != -1 for widget in widgets)
        if spellbook_open:
...
def start_bank():
    ...
    if not bank_object or 'data' not in bank_object or not bank_object['data']:
        ...
        # camera(pitch=216, yaw=151, zoom=294)
        # time.sleep(0.5)  # Brief wait for camera adjustment
        click_minimap_tile(3185, 3436, target_zoom=2)  # Click near Varrock West Bank on minimap
